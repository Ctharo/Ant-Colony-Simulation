...
static func set_category_enabled(category: Category, enabled: bool = true, from: String = "") -> void:
	var setting_key = "category_" + CATEGORY_NAMES[category].to_lower()
	SettingsManager.set_setting(setting_key, enabled)
	DebugLogger.info(Category.PROGRAM, "%s logging category %s" % [
...
static func set_log_level(level: LogLevel, from: String = "") -> void:
	SettingsManager.set_setting("log_level", level)
	DebugLogger.info(Category.PROGRAM, "Set log level to %s" % LogLevel.keys()[level],
		{"from": from if from else "debug_logger"})

## Enable or disable context print
static func set_show_context(enabled: bool = true, from: String = "") -> void:
	SettingsManager.set_setting("show_context", enabled)
	DebugLogger.info(Category.PROGRAM, "%s context display" % ["Enabled" if enabled else "Disabled"],
		{"from": from if from else "debug_logger"})
#endregion
...
static func should_log(source: String, category: Category) -> bool:
	# Get category enabled state from settings
	var setting_key = "category_" + CATEGORY_NAMES[category].to_lower()
	if not SettingsManager.get_setting(setting_key, false):
		return false
...
static func log(level: LogLevel, category: Category, message: String, context: Dictionary = {}) -> void:
	...
	var current_log_level = SettingsManager.get_setting("log_level", LogLevel.INFO)
	...
	var show_context = SettingsManager.get_setting("show_context", false)
...
static func is_trace_enabled() -> bool:
	return LogLevel.TRACE <= SettingsManager.get_setting("log_level", LogLevel.INFO)

static func is_debug_enabled() -> bool:
	return LogLevel.DEBUG <= SettingsManager.get_setting("log_level", LogLevel.INFO)
