class_name QuadTree
extends RefCounted

## Inner class for representing quadtree bounds
class QuadTreeBounds:
	var center: Vector2
	var size: Vector2

	func _init(p_center: Vector2, p_size: Vector2) -> void:
		center = p_center
		size = p_size

	## Check if a point is within the bounds
	func contains(point: Vector2) -> bool:
		return (
			point.x >= center.x - size.x / 2 and
			point.x <= center.x + size.x / 2 and
			point.y >= center.y - size.y / 2 and
			point.y <= center.y + size.y / 2
		)

	## Check if this bounds intersects with another bounds
	func intersects(other: QuadTreeBounds) -> bool:
		return not (
			other.center.x - other.size.x / 2 > center.x + size.x / 2 or
			other.center.x + other.size.x / 2 < center.x - size.x / 2 or
			other.center.y - other.size.y / 2 > center.y + size.y / 2 or
			other.center.y + other.size.y / 2 < center.y - size.y / 2
		)

## Inner class for objects stored in the quadtree
class QuadTreeObject:
	var position: Vector2
	var data: Variant

	func _init(p_position: Vector2, p_data: Variant) -> void:
		position = p_position
		data = p_data

## Member variables
var bounds: QuadTreeBounds
var objects: Array[QuadTreeObject] = []
var children: Array[QuadTree] = []
var max_objects: int = 10
var max_depth: int = 5
var depth: int = 0

## Initialize the quadtree with bounds and optional depth
func _init(p_bounds: QuadTreeBounds, p_depth: int = 0) -> void:
	bounds = p_bounds
	depth = p_depth

## Clear all objects and children from the quadtree
func clear() -> void:
	objects.clear()
	for child in children:
		child.clear()
	children.clear()

## Check if the quadtree is empty (no objects in this node or children)
func is_empty() -> bool:
	if not objects.is_empty():
		return false
	for child in children:
		if not child.is_empty():
			return false
	return true

## Subdivide the current node into four children
func subdivide() -> void:
	var half_size = bounds.size / 2
	var quarter_size = half_size / 2

	for x in [-1, 1]:
		for y in [-1, 1]:
			var center = bounds.center + Vector2(quarter_size.x * x, quarter_size.y * y)
			var child_bounds = QuadTreeBounds.new(center, half_size)
			children.append(QuadTree.new(child_bounds, depth + 1))

## Insert an object into the quadtree
func insert(position: Vector2, data: Variant) -> void:
	if not bounds.contains(position):
		return

	# If we haven't reached max objects or max depth, add to current node
	if objects.size() < max_objects or depth >= max_depth:
		objects.append(QuadTreeObject.new(position, data))
		return

	# If we haven't subdivided yet, do so
	if children.is_empty():
		subdivide()

		# Redistribute existing objects
		var current_objects = objects.duplicate()
		objects.clear()
		for obj in current_objects:
			for child in children:
				child.insert(obj.position, obj.data)

	# Add new object to appropriate children
	for child in children:
		child.insert(position, data)

## Query objects within a rectangular range
func query_range(range_bounds: QuadTreeBounds) -> Array:
	var found_objects: Array = []

	if not bounds.intersects(range_bounds):
		return found_objects

	# Add objects in current node that fall within range
	for obj in objects:
		if range_bounds.contains(obj.position):
			found_objects.append(obj.data)

	# If we have children, query them as well
	if not children.is_empty():
		for child in children:
			found_objects.append_array(child.query_range(range_bounds))

	return found_objects

## Query objects within a radius of a point
func query_radius(center: Vector2, radius: float) -> Array:
	# Create a bounding box for initial filtering
	var range_bounds = QuadTreeBounds.new(
		center,
		Vector2.ONE * radius * 2
	)

	var found_objects: Array = []
	var candidates = query_range(range_bounds)

	# Filter candidates by actual radius
	for obj in candidates:
		var obj_pos: Vector2 = obj.position if obj is Dictionary and obj.has("position") else obj.world_pos
		if center.distance_to(obj_pos) <= radius:
			found_objects.append(obj)

	return found_objects
