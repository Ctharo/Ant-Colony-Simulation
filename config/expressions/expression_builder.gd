...
func create_is_food_visible_condition(entity: Node) -> Condition:
	...
	# position2_expression will be set dynamically to each food item's position
	...
	var is_food_visible = ListAnyExpression.new()
	is_food_visible.id = "is_food_visible"
	is_food_visible.name = "Is Food Visible"
	is_food_visible.array_expression = visible_food
	is_food_visible.condition_expression = food_distance

	# Initialize all expressions with entity and cache manager
	...
	is_food_visible.initialize(entity, cache_manager)
	...
	condition.logic_expression = is_food_visible
