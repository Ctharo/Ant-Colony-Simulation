...
func _connect_signals() -> void:
	...
	if not expression_edit.text_changed.is_connected(_on_expression_changed):
		expression_edit.text_changed.connect(_on_expression_changed)
	if not type_option.item_selected.is_connected(_on_type_selected):
		type_option.item_selected.connect(_on_type_selected)
	if not description_edit.text_changed.is_connected(_on_description_changed):
...
func _show_nested_expression_editor(logic: Logic) -> void:
	...
	var editor = load("res://ui/logic_editor/logic_editor_ui.tscn").instantiate()
