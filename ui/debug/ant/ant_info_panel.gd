...
## Check if an influence should be skipped in the display
func _should_skip_influence(influence: Influence) -> bool:
	# Skip if influence type is in ignore list
	...
	if influence_type in STYLE.INFLUENCE_SETTINGS.IGNORE_TYPES:
		return true
	# Skip if influence is not valid for this ant
...
func _update_influences() -> void:
	...
	if not current_ant.influence_manager or not current_ant.influence_manager.active_profile:
		return

	# First pass: collect influence data and calculate total magnitude
	var total_magnitude = 0.0
	var influence_data = []

	for influence in current_ant.influence_manager.active_profile.influences:
		if _should_skip_influence(influence):
			continue
		
		var direction = EvaluationSystem.get_value(influence, current_ant)
		var magnitude = direction.length()
		
		# Skip influences with negligible magnitude
	...
	# Sort by magnitude for consistent ordering (highest first)
	influence_data.sort_custom(
		func(a, b): return a.magnitude > b.magnitude
	)

	# Second pass: add entries with normalized weights
	for data in influence_data:
		var normalized_weight = data.magnitude / total_magnitude if total_magnitude > 0 else 0.0
		_add_influence_entry(data.influence, normalized_weight)

## Add a single influence entry to the influences container
func _add_influence_entry(influence: Influence, normalized_weight: float) -> void:
